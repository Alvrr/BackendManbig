...
func PengirimanRoutes(app *fiber.App) {
	...
	// Create: admin, kasir
	g.Post("/", middleware.RoleGuard("admin", "kasir"), controllers.CreatePengiriman)
	// Update: admin, kasir, driver (driver only own)
	g.Put("/:id", middleware.RoleGuard("admin", "kasir", "driver"), controllers.UpdatePengiriman)
	// Delete: admin, kasir
	g.Delete("/:id", middleware.RoleGuard("admin", "kasir"), controllers.DeletePengiriman)
}
